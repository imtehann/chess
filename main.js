...
function enterAnalysis() {
  ...
  AnalysisBoardRenderer.draw(App.analysisChess);
  UI.buildAnalysisMoveList([], -1, null);
  UI.updateAccuracy(null, null);
}

function enterAnalysisFromGame() {
...
function loadPGN() {
  ...
  try {
    ...
    App.analysisMoveIdx = state.history.length - 1;
    renderAnalysisPosition();
    runFullAnalysis();
  } catch (e) {
...
function navigateAnalysis(idx) {
  ...
  const maxIdx = App.analysisChess.history.length - 1;
...
function renderAnalysisPosition() {
  const full = App.analysisChess;
  if (!full) return;

  // Rebuild board at analysisMoveIdx
  const target = App.analysisMoveIdx;
  const state = ChessEngine.createState();
  for (let i = 0; i <= target && i < full.history.length; i++) {
    const mv = full.history[i];
    ChessEngine.makeMove(state, mv.fr, mv.ff, mv.tr, mv.tf, mv.promotion, mv.flag);
  }

  // Build visual
  AnalysisBoardRenderer.drawState(state, full.history[target] || null);
  UI.buildAnalysisMoveList(full.history, target, (idx) => {
    App.analysisMoveIdx = idx;
    renderAnalysisPosition();
  });

  // Show eval for this position
  const evalScore = full.history[target]?.eval;
  if (evalScore !== undefined && evalScore !== null) UI.updateEval(evalScore);
}

function runFullAnalysis() {
  const history = App.analysisChess?.history;
  ...
  function analyseNext() {
    if (idx >= history.length || App.mode !== 'analysis') return;
    const state = ChessEngine.createState();
    for (let i = 0; i <= idx; i++) {
      const mv = history[i];
      ChessEngine.makeMove(state, mv.fr, mv.ff, mv.tr, mv.tf, mv.promotion, mv.flag);
    ...
    StockfishEngine.getEvaluation(fen, 14, (ev) => {
      if (!ev) { idx++; analyseNext(); return; }
      const score = state.turn === 'black' ? ev.score : -ev.score;
      history[idx].eval = score;
      evals.push(score);

      // Classify
      if (idx > 0) {
        const prevScore = history[idx - 1].eval ?? 0;
        const wasWhite = idx % 2 === 1;
        history[idx].cls = StockfishEngine.classifyMove(prevScore, score, !wasWhite);
      }
  ...
  // Run in background
  setTimeout(analyseNext, 200);

  // Accuracy computation (after all done)
  const checkDone = setInterval(() => {
    const done = history.every(m => m.eval !== undefined && m.eval !== null);
    if (done) {
      clearInterval(checkDone);
      computeAccuracy();
      const evalArr = history.map(m => m.eval ?? 0);
      UI.drawEvalGraph(document.getElementById('eval-graph-canvas'), evalArr);
      UI.buildAnalysisMoveList(history, App.analysisMoveIdx, (i) => {
        App.analysisMoveIdx = i;
        renderAnalysisPosition();
      });
    }
  }, 800);
...
function computeAccuracy() {
  ...
  const classWeights = { brilliant: 100, great: 95, good: 90, inaccuracy: 65, mistake: 40, blunder: 10 };

  history.forEach((mv, i) => {
    const isWhite = i % 2 === 0;
    const w = classWeights[mv.cls] ?? 80;
    if (isWhite) { whiteSum += w; whiteCount++; }
    else { blackSum += w; blackCount++; }
  });

  const wAcc = whiteCount ? whiteSum / whiteCount : 0;
  const bAcc = blackCount ? blackSum / blackCount : 0;
  UI.updateAccuracy(wAcc, bAcc);
}
