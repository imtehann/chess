...
const Board = (() => {
  ...
  function animatePieceMove(fromSq, toSq, callback) {
    // render() has already run before this is called, so the piece now lives at toSq
    const toEl = document.querySelector(`[data-square="${toSq}"] .piece`);
    if (toEl) toEl.classList.add('bloom-flash');
